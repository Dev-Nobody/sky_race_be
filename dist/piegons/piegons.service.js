...
let PigeonsService = class PigeonsService {
  ...
  async markStart(id) {
    ...
    if (!pigeon) throw new common_1.NotFoundException("Pigeon not found");
  ...
  async markLanding(id) {
    ...
    if (!pigeon || !pigeon.start_time)
      throw new common_1.NotFoundException(
        "Pigeon not found or start time missing"
      );
    pigeon.end_time = new Date();
    pigeon.duration_seconds = Math.floor(
      (pigeon.end_time.getTime() - pigeon.start_time.getTime()) / 1000
    );
    return await this.pigeonRepo.save(pigeon);
  }
