...
const pigeon_entity_1 = require("src/db/entities/pigeon.entity");
...
let PigeonsService = class PigeonsService {
    ...
    async markStart(id) {
        ...
        pigeon.start_time = new Date();
        return await this.pigeonRepo.save(pigeon);
    }
    async markLanding(id) {
        const pigeon = await this.pigeonRepo.findOne({ where: { id } });
        if (!pigeon || !pigeon.start_time)
            throw new common_1.NotFoundException("Pigeon not found or start time missing");
        pigeon.end_time = new Date();
        pigeon.duration_seconds = Math.floor((pigeon.end_time.getTime() - pigeon.start_time.getTime()) / 1000);
        return await this.pigeonRepo.save(pigeon);
    }
...
//# sourceMappingURL=piegons.service.js.map